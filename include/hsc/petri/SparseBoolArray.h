...
public:
  ...
  SparseBoolArray (const std::vector<bool> &marks)
  {
    ...
    mCap = count_if (marks.begin (), marks.end (),
                     [] (const size_t &e) {return e != 0;});
    mKeys = new size_t[mCap];
    mSize = 0;
...
private:
  ...
  static std::pair<size_t*, size_t> append (size_t *array, size_t aCap,
                                            size_t currentSize, size_t element)
  {
   ...
   */
  static std::pair<size_t*, size_t> insert (size_t *array, size_t aCap,
                                            ...
                                            size_t element)
  {
    ...
    if (currentSize + 1 <= aCap) {
      memmove (array + index + 1, array + index,
               (currentSize - index) * sizeof(size_t));
      array[index] = element;
      return {array,aCap};
    }
    ...
    memcpy (newArray + index + 1, array + index,
            (aCap - index) * sizeof(size_t));
    delete[] array;
    return {newArray,nCap};
  }

  static bool equalsRange (const size_t *const a, const size_t *const b,
                           size_t s)
  {
