...
std::vector<unit_domain> analyze_domains(const std::vector<datum>& forms) {
  ...
  std::vector<std::optional<std::pair<xpl::value, xpl::value>>> declared;
  declared.reserve(s.order().size());
  for (std::uint32_t p = 0; p < s.order().size(); ++p) {
    declared.push_back(s.bound(p));
  }
  for (auto& r : xpl::infer_domains(m, seeds, groups, declared)) {
    unit_domain u;
