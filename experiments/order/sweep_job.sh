...
#   experiments/order/sweep_job.sh <instance> <CTLC|CTLF|RC|RF|RD> [heuristics.tsv]
# Environment: SWEEP_BUDGET (s per run, default 300), SWEEP_MEM (ulimit -v kB,
# default 15000000 — the 15 GB rule), SWEEP_BIN (hsc-pn), SWEEP_INPUTS
...
case $EX in
  ...
  RF) XML=ReachabilityFireability;; RD) XML=ReachabilityDeadlock;; *) echo "unknown exam $EX"; exit 2;;
esac
...
[ -f "$xml" ] && [ -f "$orc" ] && [ -f "$pnml" ] || { echo -e "$INST\t$family\t$EX\t-\t$TAG\t$states\t\t\t\t\tno-input" >> "$TSV"; exit 0; }
grep -v '^#' "$HEUR" | while IFS=$'\t' read -r name flags envs; do
  ...
  ( ulimit -v "$MEM"; env $envs_ /usr/bin/time -f "%e %U %S %M" -o "$tm" timeout "$BUDGET" "$BIN" -i "$pnml" --props "$xml" --totalTime $((BUDGET-2)) --printUnknown -q -v $flags > "$out" 2> "$err" ); rc=$?
