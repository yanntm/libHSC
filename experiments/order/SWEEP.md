...
## 9. The first sweep, read

`sweep1`: 1448 models × {SS, CTLC, CTLF}, 17 heuristics of 300 s in
sequence per job, one core, `tall%`. Of 4344 jobs, **1692 ran** (981 SS,
711 CTLC, 0 CTLF) before every `tall` node went to other users' whole-node
jobs; the 2651 still waiting were cancelled rather than held for a
scheduler estimate four days out. The record is what those 1692 jobs wrote:
40 614 rows, 35 808 scored runs over 2109 (instance, exam) pairs, 2085 of
them with every heuristic. Virtual best over those: 7527 answered.
`sweep_summary.py` on the merged TSV is the table; CTLF is absent and the
SS and CTLC legs are two thirds and half of their instances, so read shares,
not totals.

### 9.1 A verdict off a partial reachable set is unsound

Every heuristic shows nonzero `wrong`, from 25 (`force-rev`) to 101
(`random`). This is not the scorer: on `FMS-PT-10000` under `louvain` the
run says `R partial`, `partial=1`, `reach_states=8764` after the 216 s the
budget left it, and then answers `FALSE` on formula 10, which is `TRUE`.

Over the sweep, **545 of the 559 wrong CTLC verdicts (97 %) come from a run
whose `R` was partial**, and of the 1153 runs that answered anything on a
partial `R`, **545 — 47 % — were wrong**. The apparent ranking of the
shapes by `wrong` (`random` 101, `rcm` 74, `sloan-force` 58 against
`force-iters` 38) is that same fact once removed: a shape that fails to
close `R` answers anyway, and coins a wrong verdict about half the time.

A truncated `R` under-approximates. A witness found in it is real, so an
existential leaf answered TRUE stands; nothing else does, and FALSE never
does. Until the checker knows which of its verdicts survive that, a run
that did not close `R` must answer UNKNOWN.

### 9.2 The residue: 14 wrong on a complete `R`

Separate and worse, because no budget explains them: `status=ok`, `R`
complete, and the same instance answered correctly under other shapes — so
a shape changed a verdict on a full state space.

| instance | exam | shapes |
| --- | --- | --- |
| `BridgeAndVehicles-PT-V20P20N10` | CTLC | `force`, `force-rev`, `rcm`, `rcm-force`, `sloan-force` |
| `BridgeAndVehicles-PT-V10P10N10` | CTLC | `force`, `rcm` |
| `Eratosthenes-PT-500` | CTLC | `louvain`, `louvain-force`, `rcm-force`, `sloan-force` |
| `Eratosthenes-PT-200` | CTLC | `louvain-force-rev` |
| `DNAwalker-PT-12ringLLLarge` | CTLC | `louvain-force` |
| `FileSystem-PT-N02I15B15` | CTLC | `louvain` |

Three wrong SS values remain unclassified; `--cover` and the unbounded
instances are the first thing to rule out there.

### 9.3 Two defects in the record itself

The `columns` header names 31 fields where the rows carry 33:
`reach_states` and `partial` are written by `sweep_job.sh` and unnamed in
the header, which is exactly what hid §9.1 from a reader indexing by name.
Fix the header before the next sweep.

`crash` is high on several shapes — `nupn` 226, `rcm` 205, `force-rev` 198,
`random` 179 of about 2108 runs each, against 1 for `force-iters`. Nothing
here says what they are; they are the next thing to open.
