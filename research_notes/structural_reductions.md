...
* **Computing the contract** (pipeline; the ω-side is
  ...
  *contract*, memoized across component instances, models, the corpus.
  ...
  handle comparison). Boundaries: ω-contracts of nondeterministic
  components need ω-determinization — v2, entering at the EL input;
  τ^ω divergence is invisible to the prefix language (an acceptance
  choice, not a stamp fact); the transition monoid can be |Q|^|Q| —
  always build the DFA, build the full invariant only when identity /
  caching / definability read-offs pay.
