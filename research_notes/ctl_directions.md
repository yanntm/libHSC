...
| M3 | the inverse: `int_set` local inverses (havoc over a set as the one new local term), structural term inversion, exactness test and per-event `∩ R` protection; `(pred …)`; the `restrict` leaves answer | leaves, core, surface | done; differential over 150 random models |
| M4 | `hsc-pn` answers CTLCardinality / CTLFireability: vendored `CtlFormula` to the DAG, atoms through `props_to_surface`; differential vs `its-ctl` on `examples/mcc` and PetriSpot `bench/models` `.solved` | tools | done: 96/96 verdicts on ShieldRVt, AutoFlight, Raft; Angiogenesis over the 15 s cap |
