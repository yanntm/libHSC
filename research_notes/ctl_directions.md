...
| M4 | `hsc-pn` answers CTLCardinality / CTLFireability: vendored `CtlFormula` to the DAG, atoms through `props_to_surface`; differential vs `its-ctl` on `examples/mcc` and PetriSpot `bench/models` `.solved` | tools | next |
